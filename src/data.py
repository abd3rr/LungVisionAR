...
class RALO_Dataset(Dataset):
    def __init__(self, imgpath, csvpath, subset="train", transform=None, score="GE"):
        """
        Initializes the RALO_Dataset object.

        Parameters:
        imgpath (str): Path to the directory where the images are stored.
        csvpath (str): Path to the CSV file containing the labels.
        subset (str): Indicates the subset of the dataset to be used ('train' or 'test').
        transform (callable, optional): A function/transform that takes in an PIL image and returns a transformed version.
        score (str): Type of score to be used as labels ('GE' for Geographic Extent, others for Lung Opacity).
        """
        ...
        # Process the CSV data based on the type of score
        if score == "GE":
            # Calculate the average for Geographic Extent score
            csv["Geographic Extent"] = (csv["Total GEOGRAPHIC"] + csv["Total GEOGRAPHIC.1"]) / 2
            self.labels = csv["Geographic Extent"]
        else:
            # Calculate the average for Lung Opacity score
            csv["Lung Opacity"] = (csv["Total OPACITY"] + csv["Total OPACITY.1"]) / 2
            self.labels = csv["Lung Opacity"]

        # Convert labels to a numpy array of type float32
        self.labels = np.asarray(self.labels, dtype=np.float32)
        # Create a list of image file paths
        self.images = [os.path.join(imgpath, f"{idx}.jpg") for idx in range(len(self.labels))]

        # List of IDs for the test dataset
        ...
        # Filter the dataset based on the specified subset
        if subset == "train":
            # Use all images and labels not in the test_ids for training
            self.images = [self.images[idx] for idx in range(len(self.images)) if idx not in test_ids]
            self.labels = [self.labels[idx] for idx in range(len(self.labels)) if idx not in test_ids]
        else:
            # Use only the images and labels specified in test_ids for testing
            self.images = [self.images[idx] for idx in test_ids]
            self.labels = [self.labels[idx] for idx in test_ids]

    def __str__(self):
        """ Returns a string representation of the dataset. """
        return f"RALO_Dataset({self.subset}): {len(self)} images"
...
if __name__ == "__main__":
    # Example of how to use the RALO_Dataset class
    dataset = RALO_Dataset(imgpath="data/RALO-Dataset/CXR_images_scored/", csvpath="data/RALO-Dataset/ralo-dataset-metadata.csv")
